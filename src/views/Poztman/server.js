...
let fs = require('fs');
let data = fs.readFileSync(require('path').join(__dirname, 'words.json'));
...
function updateWord(request, response) {
    ...
    if (result.reply.status == "Success")
        updateFile(result.updatedWords);
